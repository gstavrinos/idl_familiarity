...
for i=0,size(files,/n_elements)-1 do begin
    ; Q1
    dio_filename = "DIO/2015/12/DIO" + files[i]
    thl_filename = "THL/2015/12/THL" + files[i]
    if file_test(dio_filename) and file_test(thl_filename) then begin
        print, "Reading data files of day " + string(i+1) + " for question 1"
        dio_data = read_ascii(dio_filename, delimiter=',', data_start=9)
        thl_data = read_ascii(thl_filename, delimiter=',', data_start=9)
        ...
        print, "One of the necessary files was not found. Exiting..."
        break
    endelse
endfor
